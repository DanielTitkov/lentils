...
func (bq *BadgeQuery) sqlAll(ctx context.Context) ([]*Badge, error) {
	...
	if query := bq.withUsers; query != nil {
		...
		ids := make(map[int]*Badge, len(nodes))
		...
		var (
			edgeids []uuid.UUID
			edges   = make(map[uuid.UUID][]*Badge)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(badge.UsersPrimaryKey[1], fks...))
			},
