...
func (bc *BadgeCreate) check() error {
	if _, ok := bc.mutation.CreateTime(); !ok {
		return &ValidationError{Name: "create_time", err: errors.New(`ent: missing required field "Badge.create_time"`)}
	}
	if _, ok := bc.mutation.UpdateTime(); !ok {
		return &ValidationError{Name: "update_time", err: errors.New(`ent: missing required field "Badge.update_time"`)}
	}
	...
	if v, ok := bc.mutation.GetType(); ok {
		if err := badge.TypeValidator(v); err != nil {
			return &ValidationError{Name: "type", err: fmt.Errorf(`ent: validator failed for field "Badge.type": %w`, err)}
		}
	}
	if _, ok := bc.mutation.Active(); !ok {
		return &ValidationError{Name: "active", err: errors.New(`ent: missing required field "Badge.active"`)}
	}
	return nil
}
...
func (bc *BadgeCreate) createSpec() (*Badge, *sqlgraph.CreateSpec) {
	...
	if value, ok := bc.mutation.Meta(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeJSON,
			Value:  value,
