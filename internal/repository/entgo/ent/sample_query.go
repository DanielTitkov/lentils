...
//
//	client.Sample.Query().
//		GroupBy(sample.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
func (sq *SampleQuery) GroupBy(field string, fields ...string) *SampleGroupBy {
	grbuild := &SampleGroupBy{config: sq.config}
...
//
//	client.Sample.Query().
//		Select(sample.FieldCreateTime).
//		Scan(ctx, &v)
//
func (sq *SampleQuery) Select(fields ...string) *SampleSelect {
	sq.fields = append(sq.fields, fields...)
	selbuild := &SampleSelect{SampleQuery: sq}
	selbuild.label = sample.Label
...
func (sq *SampleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Sample, error) {
	...
	if query := sq.withNorms; query != nil {
		...
		nodeids := make(map[uuid.UUID]*Sample)
		for i := range nodes {
		...
		query.Where(predicate.Norm(func(s *sql.Selector) {
			s.Where(sql.InValues(sample.NormsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.sample_norms
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "sample_norms" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "sample_norms" returned %v for node %v`, *fk, n.ID)
			}
