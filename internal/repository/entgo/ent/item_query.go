...
func (iq *ItemQuery) sqlAll(ctx context.Context) ([]*Item, error) {
	...
	if query := iq.withScale; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[uuid.UUID]*Item, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(item.ScalePrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, iq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "scale": %w`, err)
		}
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.Scale = append(nodes[i].Edges.Scale, n)
			}
		}
	}
	...
	if query := iq.withQuestion; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(item.QuestionPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(uuid.UUID), new(uuid.UUID)}
			},
