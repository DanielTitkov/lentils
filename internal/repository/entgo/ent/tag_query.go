...
func (tq *TagQuery) QueryTranslations() *TagTranslationQuery {
	query := &TagTranslationQuery{config: tq.config}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withTranslations; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Translations = []*TagTranslation{}
		}
		query.withFKs = true
		query.Where(predicate.TagTranslation(func(s *sql.Selector) {
			s.Where(sql.InValues(tag.TranslationsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.tag_translations
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "tag_translations" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "tag_translations" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := tq.withTest; query != nil {
		...
		query.Where(func(s *sql.Selector) {
			joinT := sql.Table(tag.TestTable)
			s.Join(joinT).On(s.C(test.FieldID), joinT.C(tag.TestPrimaryKey[0]))
			s.Where(sql.InValues(joinT.C(tag.TestPrimaryKey[1]), edgeids...))
			columns := s.SelectedColumns()
			s.Select(joinT.C(tag.TestPrimaryKey[1]))
