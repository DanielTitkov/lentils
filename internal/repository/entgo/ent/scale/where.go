...
func HasInterpretations() predicate.Scale {
	return predicate.Scale(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(InterpretationsTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, InterpretationsTable, InterpretationsColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}
...
func HasInterpretationsWith(preds ...predicate.Interpretation) predicate.Scale {
	return predicate.Scale(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(InterpretationsInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, InterpretationsTable, InterpretationsColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
